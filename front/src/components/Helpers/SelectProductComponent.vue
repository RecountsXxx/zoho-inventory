  ...
  methods: {
    addProduct() {
      this.localProducts.push({
        ...
        rate: 0,
        tax_id: null,
        index: this.localProducts.length,
      });
    },
    removeProduct(index) {
    ...
    selectProduct(index) {
      ...
      if (selectedItem) {
        this.localProducts[index].rate = selectedItem.rate;
        this.localProducts[index].tax_id = selectedItem.tax_id;
        if (!this.showNameInput) {
          this.localProducts[index].name = selectedItem.name;
        }
      } else {
        this.localProducts[index].rate = 0;
        this.localProducts[index].tax_id = '';
        if (!this.showNameInput) {
          this.localProducts[index].name = '';
        }
      }
    },
    selectTax(index) {
      ...
      for (let i = 0; i < this.localProducts.length; i++) {
        if (i !== index && this.localProducts[i].tax_id === selectedTaxId) {
          this.localProducts[i].tax_id = null;
        }
      }
    },
  }
